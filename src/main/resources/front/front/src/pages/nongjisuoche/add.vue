<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="锁车时间" prop="suocheshijian">
				<el-date-picker
					:disabled=" false  ||ro.suocheshijian"
			...
			<el-form-item class="add-item" label="锁车原因" prop="suocheyuanyin">
				<el-input v-model="ruleForm.suocheyuanyin" 
					placeholder="锁车原因" clearable :disabled=" false  ||ro.suocheyuanyin"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`nongjisuoche/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`nongjisuoche/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
