	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="租赁编号" prop="zulinbianhao">
				<el-input v-model="ruleForm.zulinbianhao" placeholder="租赁编号" disabled></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="租赁天数" prop="zulintianshu">
				<el-input v-model.number="ruleForm.zulintianshu" 
					placeholder="租赁天数" clearable :disabled=" false  ||ro.zulintianshu"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="租赁金额" prop="zulinjine">
				<el-input v-model="zulinjine" placeholder="租赁金额" disabled></el-input>
			</el-form-item>
      ...
      <el-form-item class="add-item" label="收货地址" prop="address">
        <el-input v-model.number="ruleForm.address"
                  placeholder="收货地址" clearable ></el-input>
      </el-form-item>
			<el-form-item class="add-item"  label="归还状态" prop="guihaizhuangtai">
				<el-select v-model="ruleForm.guihaizhuangtai" placeholder="请选择归还状态" :disabled="true  ||ro.guihaizhuangtai" >
					<el-option
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`nongjizulin/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`nongjizulin/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
									duration: 1500,
									onClose: () => {
										if(this.isBackAuth('nongjizulin','支付')&&this.type=='cross'){
											this.$confirm('是否跳转支付？') .then(_ => {
												let jumpParams = {
													id: res.data.data,
													centerType: 1
												}
												this.$router.push({path: '/index/nongjizulinDetail', query: jumpParams});
											}).catch(_ => {
												this.$router.go(-1);
											});
										}else {
