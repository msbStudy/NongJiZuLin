	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'Ξ'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/nongjizulin?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.nongjileixing}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">押金</div>
						<div class="text "  >{{detail.yajin}}</div>
					</div>
					<div class="item">
						<div class="lable">日租价格</div>
						<div class="text "  >{{detail.rizujiage}}</div>
					</div>
					<div class="item">
						<div class="lable">租赁天数</div>
						<div class="text "  >{{detail.zulintianshu}}</div>
					</div>
					<div class="item">
						<div class="lable">租赁金额</div>
						<div class="text "  >{{detail.zulinjine}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">归还状态</div>
						<div class="text "  >{{detail.guihaizhuangtai}}</div>
					</div>
					<div class="item" v-if="btnAuth('nongjizulin','支付')">
          ...
          <div class="item">
            ...
            <div class="text "  >{{detail.address}}</div>
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('nongjizulin','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('nongjizulin','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('nongjizulin','归还')" 
							@click="onAcross('nongjiguihai','否','是','guihaizhuangtai','已归还','已归还,未归还'.split(',')[0])" type="warning">
							归还
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('nongjizulin','维修申请')" 
							@click="onAcross('weixiushenqing','否','是','','')" type="warning">
							维修申请
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('nongjizulin','农机锁车')" 
							@click="onAcross('nongjisuoche','否','是','','')" type="warning">
							农机锁车
						</el-button>
						<el-button class="payBtn" v-if="(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('nongjizulin','支付')" type="warning" size="small" @click="payClick">支付</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`nongjizulin`);
			...
			payClick(){
				localStorage.setItem('paytable','nongjizulin')
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/nongjizulin', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/nongjizulinAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此农机租赁？') .then(_ => {
					this.$http.post('nongjizulin/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
