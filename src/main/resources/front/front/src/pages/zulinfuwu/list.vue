	...
	<div>
		...
		<div class="list-preview">
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('zulinfuwu','新增')" type="primary" @click="add('/index/zulinfuwuAdd')">
				...
				<div class="list6">
					<div v-for="(item,index) in dataList" :key="index" :class="'list-item' + ((index%2) + 1)" @click.stop="toDetail(item)">
						...
						<div class="infoBox">
							...
							<div class="name">日租价格:{{item.rizujiage}}</div>
							<div class="name">农机状态:{{item.nongjizhuangtai}}</div>
...
<script>
	export default {
		...
		async created() {
			...
			this.selectOptionsList.push({name:'农机状态',list:this.nongjizhuangtaiOptions,tableName: 'nongjizhuangtai',check: -1})
		...
		methods: {
			...
			async getFenlei() {
				await this.$http.get('option/nongjileixing/nongjileixing').then(res => {
					if (res.data.code == 0) {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.nongjizhuangtai != '') searchWhere.nongjizhuangtai = this.formSearch.nongjizhuangtai;
				if (this.curFenlei != '全部') searchWhere.nongjileixing = this.curFenlei;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`zulinfuwu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/zulinfuwuDetail', query: params});
			},
