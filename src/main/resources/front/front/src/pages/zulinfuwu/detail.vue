	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'Ξ'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zulinfuwu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">性能参数</div>
						<div class="text "  >{{detail.xingnengcanshu}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.nongjizhuangtai}}</div>
					</div>

					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zulinfuwu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zulinfuwu','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('zulinfuwu','租赁')" 
							@click="onAcross('nongjizulin','','','nongjizhuangtai','已租赁','已租赁,空闲'.split(',')[0])" type="warning">
							租赁
						</el-button>
					</div>
				</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="农机详情" name="first">
					<div class="ql-snow ql-editor" v-html="detail.nongjixiangqing"></div>
				</el-tab-pane>
			</el-tabs>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.nongjimingcheng;
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`zulinfuwu`);
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('zulinfuwu/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'zulinfuwu', userid: localStorage.getItem('frontUserid')}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/zulinfuwu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/zulinfuwuAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此租赁服务？') .then(_ => {
					this.$http.post('zulinfuwu/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
