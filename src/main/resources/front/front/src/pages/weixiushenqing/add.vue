	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="报修编号" prop="baoxiubianhao">
				<el-input v-model="ruleForm.baoxiubianhao" placeholder="报修编号" disabled></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`weixiushenqing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			// 提交
			async onSubmit() {
				if(this.ruleForm.baoxiubianhao){
					this.ruleForm.baoxiubianhao = String(this.ruleForm.baoxiubianhao)
				}
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`weixiushenqing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
