	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'Ξ'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/weixiufenpei?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
				...
				<div class="info">
					<div class="title-item">
						<div class="detail-title">
							{{detail.nongjibianhao}}
						</div>
					</div>
					<div class="item">
						<div class="lable">报修编号</div>
						<div class="text "  >{{detail.baoxiubianhao}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('weixiufenpei','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('weixiufenpei','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('weixiufenpei','维修完成')" 
							@click="onAcross('weixiuwancheng','','','weixiuzhuangtai','已完成','已完成,未完成'.split(',')[0])" type="warning">
							维修完成
						</el-button>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.nongjibianhao;
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`weixiufenpei`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/weixiufenpei', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/weixiufenpeiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此维修分配？') .then(_ => {
					this.$http.post('weixiufenpei/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
