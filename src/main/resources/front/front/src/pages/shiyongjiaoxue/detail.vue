	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'Ξ'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/shiyongjiaoxue?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">操作手册</div>
						<el-button class="uploadBtn" @click="download(detail.caozuoshouce )">点击下载</el-button>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('shiyongjiaoxue','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('shiyongjiaoxue','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
			...
			<video v-if="detail.caozuoshipin" ref="caozuoshipin" :src="baseUrl +  detail.caozuoshipin"  controls="controls"  controlsList="nodownload">
				您的浏览器不支持视频播放
			</video>
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('shiyongjiaoxue/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'shiyongjiaoxue', userid: localStorage.getItem('frontUserid')}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/shiyongjiaoxue', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/shiyongjiaoxueAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此使用教学？') .then(_ => {
					this.$http.post('shiyongjiaoxue/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
