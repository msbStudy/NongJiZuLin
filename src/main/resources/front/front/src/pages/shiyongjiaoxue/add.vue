<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="操作手册" prop="caozuoshouce">
				<file-upload
					tip="点击上传操作手册"
					action="file/upload"
					:limit="1"
					:type="3"
					:multiple="true"
					:fileUrls="ruleForm.caozuoshouce?ruleForm.caozuoshouce:''"
					@change="caozuoshouceUploadChange"
					></file-upload>
			</el-form-item>  
			<el-form-item class="add-item" label="操作视频" v-if="type!='cross' || (type=='cross' && !ro.caozuoshipin)" prop="caozuoshipin">
				<file-upload
					tip="点击上传操作视频"
					action="file/upload"
					:limit="1"
			...
			<el-form-item class="add-item" v-else label="操作视频" prop="caozuoshipin">
				<div class="viewBtn" v-if="ruleForm.caozuoshipin" @click="download(baseUrl + ruleForm.caozuoshipin)" type="success">预览</div>
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='caozuoshouce'){
							this.ruleForm.caozuoshouce = obj[o];
							this.ro.caozuoshouce = true;
							continue;
						}
						if(o=='caozuoshipin'){
			...
			// 多级联动参数
			info() {
				this.$http.get(`shiyongjiaoxue/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`shiyongjiaoxue/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
			...
			caozuoshouceUploadChange(fileUrls) {
				this.ruleForm.caozuoshouce = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
			caozuoshipinUploadChange(fileUrls) {
				this.ruleForm.caozuoshipin = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
