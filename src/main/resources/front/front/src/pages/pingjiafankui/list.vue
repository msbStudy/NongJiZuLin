	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('pingjiafankui','新增')" type="primary" @click="add('/index/pingjiafankuiAdd')">
			...
			<div class="list">
				<div class="list6">
					<div v-for="(item,index) in dataList" :key="index" :class="'list-item' + ((index%2) + 1)" @click.stop="toDetail(item)">
						...
						<div class="infoBox">
							<div class="name">{{item.zulinbianhao}}</div>
							<div class="name">{{item.nongjimingcheng}}</div>
							<div class="name">评价时间:{{item.pingjiashijian}}</div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`pingjiafankui/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/pingjiafankuiDetail', query: params});
			},
