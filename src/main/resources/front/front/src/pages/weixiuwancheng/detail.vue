	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'Ξ'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/weixiuwancheng?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">维修结果</div>
						<div class="text "  >{{detail.weixiujieguo}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('weixiuwancheng','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('weixiuwancheng','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="维修内容" name="first">
					<div class="ql-snow ql-editor" v-html="detail.weixiuneirong"></div>
				</el-tab-pane>
			</el-tabs>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/weixiuwancheng', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/weixiuwanchengAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此维修完成？') .then(_ => {
					this.$http.post('weixiuwancheng/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
