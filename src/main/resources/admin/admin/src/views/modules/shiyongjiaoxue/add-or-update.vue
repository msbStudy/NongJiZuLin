	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info'&& !ro.caozuoshouce" label="操作手册" prop="caozuoshouce" >
					<file-upload
						tip="点击上传操作手册"
						action="file/upload"
						:limit="1"
				...
				<el-form-item v-else-if="ruleForm.caozuoshouce" label="操作手册" prop="caozuoshouce" >
					<el-button class="downBtn" type="text" size="small" @click="download($base.url+ruleForm.caozuoshouce)">
				...
				<el-form-item class="upload" v-if="type!='info'&& !ro.caozuoshipin" label="操作视频" prop="caozuoshipin" >
					<file-upload
						tip="点击上传操作视频"
						action="file/upload"
						:limit="1"
	...
	export default {
		...
		methods: {
			...
			async onSubmit() {
					...
					if(this.ruleForm.caozuoshouce!=null) {
						this.ruleForm.caozuoshouce = this.ruleForm.caozuoshouce.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shiyongjiaoxue/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shiyongjiaoxueCrossAddOrUpdateFlag = false;
											this.parent.search();
			...
			caozuoshouceUploadChange(fileUrls) {
				this.ruleForm.caozuoshouce = fileUrls;
			},
