<template>
	<div class="main-content" :style='{"width":"calc(100% - 0px)","padding":"20px 30px 40px","margin":"0 auto","fontSize":"15px","background":"none"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"10px 0","margin":"0 0px 0px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0 0 5px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">农机名称</label>
						<el-input v-model="searchForm.nongjimingcheng" placeholder="农机名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
				...
				<el-row class="actions" :style='{"padding":"0 0 5px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('nongjizulin','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('nongjizulin','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('nongjizulin','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-kuaijiezhifu" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"34px"}'></span>
						批量支付
					</el-button>
					<el-button class="btn18" v-if="isAuth('nongjizulin','农机使用率')" type="success" @click="chartDialog1()">
              ...
              {{scope.row.driver}}
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('nongjizulin','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('nongjizulin','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('nongjizulin','归还')" @click="nongjiguihaiCrossAddOrUpdateHandler(scope.row,'cross','否','是','guihaizhuangtai','已归还','已归还,未归还'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								归还
							</el-button>
							<el-button class="btn8" v-if="isAuth('nongjizulin','维修申请')" @click="weixiushenqingCrossAddOrUpdateHandler(scope.row,'cross','否','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								维修申请
							</el-button>
							<el-button class="btn8" v-if="isAuth('nongjizulin','农机锁车')" @click="nongjisuocheCrossAddOrUpdateHandler(scope.row,'cross','否','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								农机锁车
							</el-button>
							<el-button class="edit" v-if=" isAuth('nongjizulin','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>

              <el-button class="edit" v-if=" isAuth('nongjizulin','修改') " type="success" @click="changeDriverHandler(scope.row.id)">
                <span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
               分配司机
              </el-button>
							...
							<el-button class="del" v-if="isAuth('nongjizulin','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<nongjiguihai-cross-add-or-update v-if="nongjiguihaiCrossAddOrUpdateFlag" :parent="this" ref="nongjiguihaiCrossaddOrUpdate"></nongjiguihai-cross-add-or-update>
		<weixiushenqing-cross-add-or-update v-if="weixiushenqingCrossAddOrUpdateFlag" :parent="this" ref="weixiushenqingCrossaddOrUpdate"></weixiushenqing-cross-add-or-update>
		<nongjisuoche-cross-add-or-update v-if="nongjisuocheCrossAddOrUpdateFlag" :parent="this" ref="nongjisuocheCrossaddOrUpdate"></nongjisuoche-cross-add-or-update>

		<el-dialog
			:visible.sync="chartVisiable1"
			width="800">
			<div id="nongjibianhaoChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import nongjiguihaiCrossAddOrUpdate from "../nongjiguihai/add-or-update";
	import weixiushenqingCrossAddOrUpdate from "../weixiushenqing/add-or-update";
	import nongjisuocheCrossAddOrUpdate from "../nongjisuoche/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				nongjiguihaiCrossAddOrUpdateFlag: false,
				weixiushenqingCrossAddOrUpdateFlag: false,
				nongjisuocheCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			nongjiguihaiCrossAddOrUpdate,
			weixiushenqingCrossAddOrUpdate,
			nongjisuocheCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			nongjiguihaiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.nongjiguihaiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','nongjizulin');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.nongjiguihaiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.nongjiguihaiCrossaddOrUpdate.init(row.id,type);
				});
			},
			weixiushenqingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptPay=='是'&&row.ispay!='已支付'){
					this.$message({
						message: "请支付完成再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.weixiushenqingCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','nongjizulin');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.weixiushenqingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.weixiushenqingCrossaddOrUpdate.init(row.id,type);
				});
			},
			nongjisuocheCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptPay=='是'&&row.ispay!='已支付'){
					this.$message({
						message: "请支付完成再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.nongjisuocheCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','nongjizulin');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.nongjisuocheCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.nongjisuocheCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				this.$storage.set('paytable','nongjizulin');
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var nongjibianhaoChart1 = echarts.init(document.getElementById("nongjibianhaoChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							nongjibianhaoChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								nongjibianhaoChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
      ...
      // 添加/修改
      changeDriverHandler(id,type) {
        this.showDriverFlag = true;
        this.showFlag = false;
        this.addOrUpdateFlag = true;
        this.crossAddOrUpdateFlag = false;
        if(type!='info'&&type!='msg'){
        ...
        this.$nextTick(() => {
          this.$refs.addOrUpdate.setshowDriver(true);
        });
